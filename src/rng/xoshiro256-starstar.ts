 ...
 */
export type Xoshiro256StarStarState = [bigint, bigint, bigint, bigint];
 ...
 * @returns {RandomNumberGenerator<Xoshiro256StarStarState>} A new PRNG.
 */
export function createRandomXoshiro256StarStar(
  seed: Seed = Date.now(),
): RandomNumberGenerator<Xoshiro256StarStarState> {
  ...
  function random(): bigint {
    const result = (rotl64(s1 * 5n, 7n) * 9n) & MASK_64;
  ...
  return defineRandomState<Xoshiro256StarStarState>(
